...
// Uses Netlify Functions as API proxy (no exposed keys)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    arr.forEach(s => {
      ...
      div.querySelector('.reload-btn').addEventListener('click', () => {
        ...
        oracleLocMsgs.innerHTML = `<div class="oracle-welcome">
          ...
          <p>Ask me about <strong>${s.location}</strong>.</p>
        </div>`;
      });
  ...
  function showOracleSuggestions(location) {
    ...
    oracleSugg.querySelectorAll('.sugg-chip').forEach(btn => {
      btn.addEventListener('click', () => { oracleInput.value = btn.textContent; sendOracleMsg(); });
    });
  }

  async function saveOracleMsg(question, answer, location) {
    if (!currentUser) return;
    await db.ref(`users/${currentUser.uid}/oracleHistory/${Date.now()}`).set({
  ...
  async function loadOracleAllHistory() {
    ...
    arr.forEach(item => {
      ...
      label.className = 'oracle-loc-label';
  ...
  async function sendOracleMsg() {
    ...
    if (!q || !window.lastResult) {
      if (!window.lastResult) addOracleMsg('Search a location first!', 'assistant', oracleLocMsgs);
      return;
    }
    oracleInput.value = '';
    oracleSugg.style.display = 'none';
...
Answer concisely and dramatically in under 150 words.`,
      ...
      thinking.remove();
      addOracleMsg('The Oracle cannot be reached right now.', 'assistant', oracleLocMsgs);
    }
  }
  ...
  async function runSearch(location) {
    ...
    try {
      // Call Netlify Function (hides API key)
  ...
  async function fetchWikipediaImages(location) {
    try {
      ...
      const valid = page.images.filter(img => {
        const n = img.title.toLowerCase();
        return !n.includes('logo') && !n.includes('icon') && !n.includes('flag') && !n.includes('map') &&
      ...
      for (const img of valid) {
        try {
          ...
          if (info?.thumburl) {
            ...
            results.push({ thumb: info.thumburl, url: info.url, desc: desc.substring(0, 80) });
          }
        } catch { /* skip */ }
      }
      return results;
    ...
    if (meta) {
      ...
      if (chunks.some(c => c.web?.uri)) {
        ...
        chunks.forEach(c => {
          ...
          el.className = 'source-card'; el.href = c.web.uri; el.target = '_blank'; el.rel = 'noopener noreferrer';
          el.innerHTML = `<span class="source-icon">🔗</span><div class="source-info"><span class="source-title">${c.web.title || c.web.uri}</span><span class="source-url">${c.web.uri}</span></div>`;
      ...
      if (img && i > 0) {
        ...
        <div class="content-with-image">
          <figure class="inline-figure">
            <a href="${img.url}" target="_blank" rel="noopener"><img src="${img.thumb}" alt="${img.desc || heading}" loading="lazy"/></a>
            ${img.desc ? `<figcaption>${img.desc}</figcaption>` : ''}
          </figure>
          <div class="inline-text">${bodyHtml}</div>
        </div>`;
      } else {
        html += i === 0 ? bodyHtml : `<h2 class="result-heading">## ${heading}</h2>${bodyHtml}`;
      }
    });
    return html;
  }
  ...
  async function buildTTSSummary(content) {
    try {
      const res  = await fetch(ORACLE_API, {
        ...
        body: JSON.stringify({
          prompt: content,
          systemPrompt: `Write a vivid dramatic audio narration of approximately 250 words about this historical location. Write like a BBC documentary narrator. No markdown. Pure flowing prose.`,
          maxTokens: 400
        })
      });
      const data = await res.json();
  ...
  function setActiveTab(tab) {
    ...
    switch(tab) {
      ...
      case 'sources':
        ...
        if (!sourcesEl.innerHTML.trim()) sourcesEl.innerHTML = '<p class="no-searches">Search a location first.</p>';
        break;
    }
  }
  ...
  async function buildTimeline() {
    ...
    try {
      const res  = await fetch(ORACLE_API, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt: window.lastResult,
          systemPrompt: `Extract exactly 8 key historical events. Return ONLY a JSON array, no markdown, no explanation.
...
Order chronologically.`,
    ...
    panel.innerHTML = `
      ...
      <div id="artifact-videos"      class="artifact-section" style="display:none;">
        <div class="artifact-grid">
          <p class="no-searches">🎥 Search documentaries on YouTube</p>
          <a href="https://www.youtube.com/results?search_query=${encodeURIComponent(window.lastLocation + ' history documentary')}" target="_blank" rel="noopener" class="yt-search-btn">▶ Search "${window.lastLocation}" on YouTube</a>
        </div>
      </div>
  ...
  async function fetchHistoricalMaps(location) {
    ...
    try {
      ...
      for (const r of data.query?.search || []) {
        if (count >= 6) break;
        const card = await fetchWikimediaCard(r.title);
  ...
  async function fetchWikimediaArtifacts(location) {
    const categories = {
      paintings:   `${location} painting portrait photograph`,
      manuscripts: `${location} manuscript document inscription`,
      others:      `${location} archaeology ruin monument`
    };
    for (const [type, query] of Object.entries(categories)) {
      ...
      try {
        ...
        for (const r of data.query?.search || []) {
          if (count >= 6) break;
          if (r.title.toLowerCase().includes('map') && type !== 'maps') continue;
          const card = await fetchWikimediaCard(r.title);
          if (card) { grid.appendChild(card); count++; }
        }
